...
void Camera::ResizeCall(GLFWwindow *w, int wid, int hig) {
    ...
    camera->Resize=true;
    camera->scrSize={camera->width/4/tan(camera->fov/2),camera->height/4/tan(camera->fov/2)};
}

glm::mat4 Camera::GetTransform(const vec3& offset)const {
    return glm::perspective(fov, width / height, .05f, 80.f) *

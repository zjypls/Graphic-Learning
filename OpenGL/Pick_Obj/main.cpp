...
Camera camera{{2, 0, 0}, {-1, 0, 0}, {0, 1, 0}, 1.5f, 1.5f, 8.f, 6.f, 1E-2, .8f, 0, 0,
              false, false, false, false, {2 / tan(.4f), 1.5 / tan(.4f)}, {0, 0},
...
int main() {
    Model::Model model{std::string{RootPath}+"/Model/cube.obj", 1, {-9, 1, 1}};
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        if (camera.WalkMode) {
            ...
            index.z = 1;
            goto paint;
        } else if (camera.MoveMode = false, index = {-1, -1, -1};false) {
            paint:
